...
function initializePlayers() {
  const boundingRect = element.getBoundingClientRect();
    ...
    for(let i=0; i< numberOfPalyers; i+=1) {
      x += 17;
      let num = i+1 + '';
      document.getElementsByClassName('player-'+ num +'-dot')[0].style.left = x +"px"
      document.getElementsByClassName('player-'+ num +'-dot')[0].style.top = y +"px"
      // positionObject[num] = [];
      ...
      li += '<li class="player-' + num + '">Player ' + num + '<span>( ' +positionObject[num]['money'] + ')</span></li>';
...
function movePlayer() {
  ...
  if(positionObject[playerTurn]['position'] + sum >39) {
    ...
    if(sum === 40) {
      ...
      let x= boundingRect.x ;
      let y = boundingRect.y;
      document.getElementsByClassName('player-'+ playerTurn +'-dot')[0].style.left = x +"px"
      document.getElementsByClassName('player-'+ playerTurn +'-dot')[0].style.top = y +"px"
  ...
  console.log('positionObject[playerTurn]', positionObject[playerTurn]['position']);
  if(!item) {
  ...
  let y = boundingRect.y ;
  for(let i=0; i< numberOfPalyers; i+=1) {
    if(positionObject[i+1]['position'] === positionObject[playerTurn]['position']) {
      y +=(playerdiff*(playerTurn -1));
    }
  }
  if(positionObject[playerTurn]['position'] >= 10 && positionObject[playerTurn]['position'] <20 ) {
    x = item.offsetWidth - 20;
    document.getElementById(player).style.left = x +"px"
    document.getElementById(player).style.top = y +"px"
  }
  else if(positionObject[playerTurn]['position'] >= 20 && positionObject[playerTurn]['position'] <30) {
    y +=20;
    x += 3; 
    document.getElementById(player).style.left = x +"px"
    document.getElementById(player).style.top = y +"px"
...
function buyOrRent() {
  const onProperty = document.getElementById(positionObject[playerTurn]['position']).innerHTML.trim();
      ...
      if(property.name === onProperty) {
        if(reg.test(property.price)) {
          if(!positionObject[property.name]){
            ...
            debugger;
            if(positionObject[property.name]['rent'] <6 && 
              positionObject[property.name]['boughtBy'] !== playerTurn) {
                positionObject[property.name]['rent'] += 1;
                debugger;
                const rent = property['rent'+positionObject[property.name]['rent']];
